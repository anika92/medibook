...
export default function AdminPending() {
  ...
  // Fixed — uppercase keys to match DB enum values
  const ROLE_COLORS = {
    ...
    <div className="admin-page">
      <div className="admin-page-header">
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          {pending.length > 0 && (
            <span style={{background:'#FEF2F2',color:'#DC2626',fontSize:11,fontWeight:700,padding:'3px 8px',borderRadius:20}}>
      ...
      {!loading && pending.length === 0 && (
        <div style={{textAlign:'center',padding:'60px 20px',background:'var(--white)',borderRadius:'var(--rl)',border:'1px solid var(--border)'}}>
          ...
          <div style={{fontSize:13,color:'var(--ink3)'}}>No pending staff registrations right now.</div>
        </div>
      )}

      <div style={{display:'flex',flexDirection:'column',gap:10}}>
        {pending.map(s => {
          const rc = ROLE_COLORS[s.role] || { bg:'#F1F5F9', tc:'#475569' }
          return (
            <div key={s.id} className="admin-table-wrap" style={{padding:'16px 20px'}}>
              <div style={{display:'flex',alignItems:'center',gap:14,flexWrap:'wrap'}}>
                <div style={{width:42,height:42,borderRadius:'50%',background:'#EEF3FF',color:'#1B4FD8',
                ...
                <div style={{display:'flex',gap:8}}>
                  <button className="btn btn-sm"
                    style={{background:'#FEF2F2',color:'#DC2626',borderColor:'#FECACA',display:'flex',alignItems:'center',gap:5}}
                    disabled={processing===s.id} onClick={()=>handleAction(s.id, false)}>
                    <XCircle size={13}/> Reject
                  </button>
                  <button className="btn btn-sm"
                    style={{background:'#ECFDF5',color:'#16A34A',borderColor:'#86EFAC',display:'flex',alignItems:'center',gap:5}}
                    disabled={processing===s.id} onClick={()=>handleAction(s.id, true)}>
                    {processing===s.id
