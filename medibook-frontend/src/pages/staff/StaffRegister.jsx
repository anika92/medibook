...
const s = {
  page:{
    ...
    backgroundImage:'radial-gradient(circle at 20% 20%, rgba(27,79,216,.06) 0%, transparent 50%)'
  },
  card:{
    width:'100%',maxWidth:500,padding:36,
    background:'var(--white)',border:'1px solid var(--border)',
    borderRadius:'var(--rl)',boxShadow:'var(--shadow)'
  },
...
export default function StaffRegister() {
  ...
  const validateField = (k, v) => {
    ...
    switch(k) {
      case 'full_name':
        ...
        else if (v.trim().length < 2) err = 'Too short'
        break
      case 'phone':
        if (!v) err = 'Phone is required'
        else if (!/^01[3-9]\d{8}$/.test(v)) err = 'Enter valid BD number (01XXXXXXXXX)'
        break
      case 'email':
        if (v && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v)) err = 'Enter a valid email (name@domain.com)'
        break
      case 'password':
        if (!v) err = 'Password is required'
        else if (v.length < 8) err = 'Minimum 8 characters'
        break
      case 'confirm':
        if (!v) err = 'Please confirm your password'
        else if (v !== form.password) err = 'Passwords do not match'
        break
    }
    setErrors(e=>({...e,[k]:err}))
    return err
  ...
  const validateAll = () => {
    ...
    fields.forEach(f => {
      allTouched[f] = true
      const val = f === 'confirm' ? form.confirm : form[f]
  ...
  const inp = (k) => ({
    ...s.input,
    ...(touched[k] && errors[k] ? s.inputError : {}),
  })
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true)
    try {
      await axios.post('/api/v1/staff-register/', {
  ...
  return (
    <div style={s.page}>
      <div style={s.card} className="fade-up">
        <div style={s.logo}>
          <div style={s.logoIcon}><Activity size={18}/></div>
          <span>MediBook Staff</span>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div style={s.group}>
            ...
            <div style={s.iw}>
              <User size={15} style={s.ic}/>
              <input style={inp('full_name')} placeholder="Dr. Rahim" value={form.full_name}
            ...
            {touched.full_name && errors.full_name
              ? <div style={s.errMsg}><span>✗</span>{errors.full_name}</div>
              : touched.full_name && form.full_name && <div style={s.okMsg}><span>✓</span>Looks good</div>
            }
          </div>
          ...
          <div style={s.group}>
            ...
            <div style={s.iw}>
              <Phone size={15} style={s.ic}/>
              <input style={inp('phone')} type="tel" placeholder="01XXXXXXXXX"
            ...
            {touched.phone && errors.phone
              ? <div style={s.errMsg}><span>✗</span>{errors.phone}</div>
              : touched.phone && !errors.phone && form.phone && <div style={s.okMsg}><span>✓</span>Valid phone number</div>
            }
          </div>
          ...
          <div style={s.group}>
            <label style={s.label}>
              Email address{' '}
              <span style={{color:'var(--ink4)',fontWeight:400,textTransform:'none',fontSize:11}}>(optional — for approval notification)</span>
            </label>
            <div style={s.iw}>
              <Mail size={15} style={s.ic}/>
              <input
                style={{...inp('email'),...(touched.email && errors.email ? s.inputError : {})}}
            ...
            {touched.email && errors.email
              ? <div style={s.errMsg}><span>✗</span>{errors.email}</div>
              : form.email && !errors.email
                ? <div style={s.okMsg}><span>✓</span>You will be notified here when approved</div>
                : <div style={{fontSize:11,color:'var(--ink4)',marginTop:4}}>Receive email when your account is approved</div>
            }
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            <div style={s.group}>
              ...
              <div style={s.iw}>
                <Lock size={15} style={s.ic}/>
                <input style={inp('password')} type="password" placeholder="Min 8 chars"
              ...
              {touched.password && errors.password
                ? <div style={s.errMsg}><span>✗</span>{errors.password}</div>
                : touched.password && form.password.length>=8 && <div style={s.okMsg}><span>✓</span>Good</div>
              }
            </div>
            <div style={s.group}>
              ...
              <div style={s.iw}>
                <Lock size={15} style={s.ic}/>
                <input style={inp('confirm')} type="password" placeholder="Repeat"
              ...
              {touched.confirm && errors.confirm
                ? <div style={s.errMsg}><span>✗</span>{errors.confirm}</div>
                : touched.confirm && form.confirm && form.confirm===form.password && <div style={s.okMsg}><span>✓</span>Match</div>
              }
            </div>
          </div>
            ...
            <div style={{marginBottom:16}}>
              <div style={{height:4,background:'var(--border)',borderRadius:2,overflow:'hidden',marginBottom:4}}>
                <div style={{
                  ...
                  background: form.password.length>=12?'#0EA472':form.password.length>=8?'#F59E0B':'#DC2626'
                }}/>
              </div>
              <div style={{fontSize:11,color:'var(--ink4)'}}>
                {form.password.length>=12?'Strong ✓':form.password.length>=8?'Good — longer is better':'Too short'}
              </div>
            </div>
          )}
          ...
          {/* NOTICE */}
          <div style={s.notice}>
            <CheckCircle size={14} style={{color:'#D97706',flexShrink:0,marginTop:2}}/>
            <span>Your account will be reviewed by a super admin. You cannot login until approved.</span>
          </div>
        ...
        <div style={s.footer}>
          Already approved?{' '}
          <Link to="/staff/login" style={{color:'var(--primary)',fontWeight:500,textDecoration:'none'}}>
            Login here
          </Link>
        </div>
      </div>
